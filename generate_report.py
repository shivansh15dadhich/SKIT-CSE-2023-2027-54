...
def generate_pdf(interval="weekly"):
    ...
    story.append(Paragraph(
        ...
        f"&nbsp;|&nbsp; <b>Branch:</b> <code>{html.escape(branch_name)}</code>", repo_style))
    story.append(Paragraph(
        f"<b>Evaluation Window:</b> {scope_title} &nbsp;|&nbsp; <b>Generated On:</b> "
        f"{datetime.date.today().strftime('%B %d, %Y')}", meta_style))
    ...
    if not student_logs:
        ...
        for student_name, logs in student_logs.items():
            student_section = []
            student_section.append(Paragraph(f"<b>Student:</b> {html.escape(student_name)} &mdash; <i>{len(logs)} commit(s)</i>", sub_section_style))
    ...
    mentor_cell = [
        Paragraph("<b>Name:</b> ________________________", sig_block_style),
